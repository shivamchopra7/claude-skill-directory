---
name: af-refine-specifications
description: Transform Linear features into detailed BDD specifications and mini-PRDs. Use when writing behaviour scenarios, creating visual designs with Storybook, defining component specs, or setting accessibility rules.
...
The refinement process transforms Linear features from Discovery into comprehensive mini-PRDs with Behaviour scenarios, visual designs, and complete technical specifications. This is the critical bridge between understanding what to build (Discovery) and building it (Delivery).
...
**Refinement work happens on the `specs` branch** (see ADR-009).
...
The `specs` branch is shared by PM, UX, and QA for all pre-implementation work. During Refinement:
...
**When Refinement complete (before approval):**
...
- All Refinement artifacts go directly to the trunk branch
...
See [Atomic Refinement Guide](../../docs/guides/atomic-refinement-guide.md) for outside-in development and full hierarchy.
...
**Command:** `/refinement:refine LIN-123`
...
Once the approach is approved, load `af-estimate-effort` and produce an initial refined estimate:
...
For each parent issue entering Refinement, create sub-issues for specification work.

**[Behaviour] and [UX] sub-issues are exempt from the sub-issue size policy** — they serve a structural BDD purpose and are always created regardless of parent size.
...
**Other sub-issues (non-spec):** Only create additional sub-issues when the parent exceeds 2 days (12 hours) of human effort, each sub-issue is ≥2 days, and the work is genuinely independent (different person, different phase, or independently deliverable). Otherwise, use a checklist in the parent issue description. See `af-manage-work-state` for the full sub-issue creation policy.
...
| Does the mini-PRD include effort estimates? | ✅ | Load `af-estimate-effort`, finalize estimate | Story points + hours set on Linear issue, Section 7 complete |
...
**Command:** `/refinement:approve LIN-123`
...
After approval, create child issues in Linear for each chunk (via GraphQL API - see `af-query-linear-api`):
...
**IMPORTANT:** Technical decisions made during Refinement phase may warrant ADRs. This is especially true when you ask the user to choose between approaches during Three Amigos analysis.

**Decision Framework (from af-decide-architecture):**
...
→ Load `af-decide-architecture` for full ADR workflow and templates
...
### Refinement phase Creates (Specifications)
...
- **Refinement** creates scenarios (specifications) in mini-PRD
...
When scenarios have preconditions like "Given a Super Admin account exists", Refinement phase must specify seed data:
...
5. **Missing RTL tests**
   - Refinement phase creates RTL tests
...
- Read `.claude/docs/guides/refinement-guide.md` (full workflow with examples)
