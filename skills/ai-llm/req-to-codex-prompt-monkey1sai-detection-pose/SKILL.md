...
description: |
  將使用者的自然語言需求，轉換成「Codex 可直接執行」的工程指令 prompt。
  輸出內容包含：背景、明確目標、可驗收條件、操作步驟、不可違反的限制、與交付格式。
  適用於任何語言、任何 repo、任何專案階段。
examples:
  - input: |
      我想讓服務可以一行指令啟動，並且能被健康檢查驗證。
    output: |
      # change prompt v YYYY.MM.DD
      你現在在一個 repo：...
      【目標與驗收條件】...
      【你需要做的事情】...
      【約束】...
