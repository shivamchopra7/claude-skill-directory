...
Read `.matclaw/state.yaml`. If `ollama` is in `applied_skills`, skip to Phase 3 (Configure). The code changes are already in place.
...
```bash
tail -f logs/matclaw.log | grep -i ollama
```
