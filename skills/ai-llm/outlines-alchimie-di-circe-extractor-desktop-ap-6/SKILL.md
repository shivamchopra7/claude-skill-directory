...
# Load local model via Docker Model Runner
model = llamacpp(
    "./models/llama-3.1-8b.Q4_K_M.gguf",
    n_ctx=4096,
    n_gpu_layers=35
...
async def generate_caption_cloud(image_description: str) -> CaptionSchema:
    # Use Instructor with cloud provider
...
class HybridCaptionGenerator:
    ...
    async def generate(self, prompt: str, schema: type):
        ...
        if self.use_local:
            ...
            # Use cloud provider
            generator = outlines.generate.json(self.cloud_client, schema)
            result = generator(prompt)
            logger.info(f"Generated caption using cloud model")
            return result
    
    def reset_to_local(self):
        """Reset to use local model after successful generation"""
        self.use_local = True
        logger.info("Switched back to local model")
```
...
async def benchmark_caption_generation():
    ...
    print(f"Local is {speedup:.0%} of cloud time")
```
