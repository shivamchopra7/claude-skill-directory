...
```text
/skill:phx-review
/skill:phx-review test
/skill:phx-review security
/skill:phx-review .claude/plans/auth/plan.md
/skill:phx-review --no-requirements
```
...
Native Pi subagents may run independent read-only concern tracks in parallel.
...
run. Stop after presenting the review. Suggest `/skill:phx-triage`, `/skill:phx-plan`, or
`/skill:phx-compound` as optional next steps without invoking them automatically.

## References

- `references/requirements-detection.md` — requirements source and coverage rules
- `references/agent-spawning.md` — Pi concern selection and optional parallelism
