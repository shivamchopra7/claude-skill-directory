...
```
/skill:phx-pr-review 42                  # Triage unresolved threads on PR #42
/skill:phx-pr-review 42 --fix            # Triage + apply approved code fixes
/skill:phx-pr-review https://...         # Full URL also works (repo parsed from URL)
/skill:phx-pr-review 42 --bots-only      # Triage only CI bot threads (Copilot, Codex...)
/skill:phx-pr-review 42 --no-resolve     # Reply but leave threads open
```
...
```text
PR receives review → /skill:phx-pr-review {number}  ← YOU ARE HERE
...
- `/skill:phx-plan` — if findings reveal scope gaps
- `/skill:phx-verify` — full verification before pushing
- Re-run `/skill:phx-pr-review` after the next review round (idempotent)
