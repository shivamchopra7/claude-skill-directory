...
`/skill:phx-permissions [--days=14] [--dry-run]` — Scans session JSONL files, finds uncovered Bash commands,
classifies risk, and recommends `settings.json` changes. Use `--dry-run` to preview without writing.
...
`.claude/reviews/`. If these aren't covered, `/skill:phx-compound` and review agents
...
Long mix output flooding context? `/skill:phx-mix-compression` installs rtk filters
