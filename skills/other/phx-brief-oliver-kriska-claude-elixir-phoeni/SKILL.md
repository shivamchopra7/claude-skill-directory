...
```
/skill:phx-brief                                    # Latest plan
/skill:phx-brief .claude/plans/user-auth/plan.md    # Specific plan
```
...
3. If no plan found, tell user and suggest `/skill:phx-plan`
...
5. **Keep each section under 20 lines** — this is a briefing,
   not a lecture
6. **NEVER skip sections or auto-start work** — briefing is read-only; do not execute plan tasks or launch `/skill:phx-work` without explicit user request
...
Ready to proceed? Try `/skill:phx-work {plan_path}` to start execution.
```
...
Consider `/skill:phx-compound` to capture key learnings for future reference.
```

## Integration

```text
/skill:phx-plan  -->  /skill:phx-brief (optional)  -->  /skill:phx-work  -->  /skill:phx-brief (optional)
