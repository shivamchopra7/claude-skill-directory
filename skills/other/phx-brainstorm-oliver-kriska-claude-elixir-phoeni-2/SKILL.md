...
`interview.md` that `/skill:phx-plan` detects and consumes (skipping clarification).

## Usage

```text
/skill:phx-brainstorm Add some kind of notification system
/skill:phx-brainstorm Improve authentication security
/skill:phx-brainstorm                    # starts with open question
```

## Workflow

```
/skill:phx-brainstorm {topic}
    |
    v
[INTERVIEW] ←──────────────────┐
    |                           |
...
[DECISION POINT]                |
    ├─ Research ──→ [RESEARCH] ─┘
    ├─ Continue interview ──────┘
    ├─ Make a plan ──→ STOP (suggest /skill:phx-plan {slug})
...
3. Use AskUserQuestion with EXACTLY these options (4 max — the tool's
   ...
   - **Make a plan** — I'll suggest: `/skill:phx-plan .claude/plans/{slug}/interview.md`
...
1. **NEVER auto-transition** to `/skill:phx-plan` — always present as option, let user choose
2. **ONE question at a time** — never dump a question list
3. **Always write artifacts** — `interview.md` is the contract with `/skill:phx-plan`
...
```
/skill:phx-brainstorm ──→ interview.md ──→ /skill:phx-plan (skips clarification)
                                 ──→ /skill:phx-plan --existing (deepens)
                                 ──→ stored for later session
```

Position: optional upstream of `/skill:phx-plan` in workflow cycle.
