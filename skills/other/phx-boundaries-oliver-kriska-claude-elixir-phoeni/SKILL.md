...
```
/skill:phx-boundaries              # Check for violations
/skill:phx-boundaries --assess     # Score context health (0-100)
/skill:phx-boundaries --fix        # Suggest fixes for violations
```
...
```
- `/skill:phx-plan` — Create a plan to fix violations (recommended for 3+ issues)
- `/skill:phx-quick` — Fix a single boundary violation directly
- `/skill:phx-review` — Review specific modules for deeper issues
```
