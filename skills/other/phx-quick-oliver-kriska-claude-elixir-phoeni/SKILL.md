...
```bash
/skill:phx-quick Add pagination to posts
/skill:phx-quick Fix the login redirect bug
/skill:phx-quick Add CSV export to reports
```
...
2. **DO NOT touch files outside the stated scope** — quick mode means minimal blast radius; if a fix requires changes across multiple domains, escalate to `/skill:phx-plan`
...
```bash
/skill:phx-quick Add phone field to users
```
...
```bash
/skill:phx-quick Fix: posts not showing for admin users
```
...
```bash
/skill:phx-quick Add GET /api/health endpoint
```
...
Run /skill:phx-plan [task] for full analysis.
```
