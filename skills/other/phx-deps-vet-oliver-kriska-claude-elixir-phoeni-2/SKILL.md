...
description: Record a vetted Hex package version in hex_vet.exs after a security review
  — manages the audit ledger, not the scanner. Use to approve a dep after /skill:phx-deps-audit
...
/skill:phx-deps-vet --check             # cross-check mix.lock vs ledger
```
...
- **Run after** `/skill:phx-deps-audit` to clear vetted findings.
- **Run before** merging a `mix.lock` PR to certify new versions.
- **Run `/skill:phx-deps-vet --check`** to detect ledger drift vs `mix.lock`.
- **`/skill:phx-deps-audit`** auto-downgrades vetted findings to INFO.
...
- `../phx-deps-audit/references/rules-impl.md` — the
  same rules `/skill:phx-deps-audit` runs
