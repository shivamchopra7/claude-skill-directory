...
# Verify: launchctl list | grep microclaw shows PID and exit code 0
```
...
| `MICROCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/microclaw.log` | Service logs (contains X operation logs) | Ignored |
...
All paths below are relative to project root (`MICROCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep microclaw  # Should show PID and exit code 0 or -
```
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/microclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/microclaw.log | tail -20
```
...
container run microclaw-agent ls -la /app/src/skills/
```
