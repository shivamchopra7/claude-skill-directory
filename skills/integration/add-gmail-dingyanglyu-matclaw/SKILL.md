---
name: add-gmail
description: Add Gmail integration to MatClaw. Can be configured as a tool (agent reads/sends emails when triggered from WhatsApp) or as a full channel (emails can trigger the agent, schedule tasks, and receive replies). Guides through GCP OAuth setup and implements the integration.
---

# Add Gmail Integration

This skill adds Gmail support to MatClaw — either as a tool (read, send, search, draft) or as a full channel that polls the inbox.

## Phase 1: Pre-flight

### Check if already applied

Read `.matclaw/state.yaml`. If `gmail` is in `applied_skills`, skip to Phase 3 (Setup). The code changes are already in place.
...
Add `gmail` to `.matclaw/state.yaml` under `applied_skills` with `mode: tool-only`.
...
>    - Application type: **Desktop app**, name: anything (e.g., "MatClaw Gmail")
...
Tell the user to send themselves a test email. The agent should pick it up within a minute. Monitor: `tail -f logs/matclaw.log | grep -iE "(gmail|email)"`.
...
3. Remove `gmail` from `.matclaw/state.yaml`
4. Clear stale agent-runner copies: `rm -r data/sessions/*/agent-runner-src 2>/dev/null || true`
5. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.matclaw` (macOS) or `systemctl --user restart matclaw` (Linux)
...
6. Remove `gmail` from `.matclaw/state.yaml`
7. Clear stale agent-runner copies: `rm -r data/sessions/*/agent-runner-src 2>/dev/null || true`
8. Rebuild: `cd container && ./build.sh && cd .. && npm run build && launchctl kickstart -k gui/$(id -u)/com.matclaw` (macOS) or `systemctl --user restart matclaw` (Linux)
