...
Then install it. **IMPORTANT:** The OpenAI SDK requires Zod v3 as an optional peer dependency, but MicroClaw uses Zod v4. This conflict is guaranteed, so always use `--legacy-peer-deps`:
...
Find the orphan cleanup block in `ensureContainerSystemRunning()` (the section starting with `// Kill and clean up orphaned MicroClaw containers from previous runs`) and replace it with:
...
```typescript
  ...
  try {
    ...
    const microclawContainers = containers.filter(
      (c) => c.configuration.id.startsWith('microclaw-'),
    );
    const running = microclawContainers
    ...
    const allNames = microclawContainers.map((c) => c.configuration.id);
...
nc = [c['configuration']['id'] for c in data if c['configuration']['id'].startswith('microclaw-')]
if nc: print(' '.join(nc))
...
Verify it started with exactly one (or zero, before first message) microclaw container:

```bash
sleep 3 && launchctl list | grep microclaw
container ls -a --format json | python3 -c "
import sys, json
data = json.load(sys.stdin)
nc = [c for c in data if c['configuration']['id'].startswith('microclaw-')]
print(f'{len(nc)} microclaw container(s)')
...
```bash
tail -f logs/microclaw.log | grep -i "voice\|transcri"
```
...
```bash
# Stop all microclaw containers, then restart the service
container ls -a --format json | python3 -c "
import sys, json
data = json.load(sys.stdin)
running = [c['configuration']['id'] for c in data if c['configuration']['id'].startswith('microclaw-') and c['status'] == 'running']
...
```bash
tail -100 logs/microclaw.log | grep -i transcription
```
...
The OpenAI SDK requires Zod v3, but MicroClaw uses Zod v4. This conflict is guaranteed — always use:
```bash
npm install --legacy-peer-deps
```
