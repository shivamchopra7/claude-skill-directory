---
name: x-integration
description: X (Twitter) integration for MatClaw. Post tweets, like, reply, retweet, and quote. Use for setup, testing, or troubleshooting X functionality. Triggers on "setup x", "x integration", "twitter", "post tweet", "tweet".
---
...
> **Compatibility:** MatClaw v1.0.0. Directory structure may change in future versions.
...
1. **MatClaw is installed and running** - WhatsApp connected, service active
...
# Verify: launchctl list | grep matclaw (macOS) or systemctl --user status matclaw (Linux)
```
...
| `MATCLAW_ROOT` | `process.cwd()` | Project root directory |
...
| `logs/matclaw.log` | Service logs (contains X operation logs) | Ignored |
...
To integrate this skill into MatClaw, make the following modifications:
...
All paths below are relative to project root (`MATCLAW_ROOT`).
...
**Verify success:**
```bash
launchctl list | grep matclaw  # macOS — should show PID and exit code 0 or -
...
grep -i "x_post\|x_like\|x_reply\|handleXIpc" logs/matclaw.log | tail -20

# Script errors
grep -i "error\|failed" logs/matclaw.log | tail -20
```
...
docker run matclaw-agent ls -la /app/src/skills/
```
