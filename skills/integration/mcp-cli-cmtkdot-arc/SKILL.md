---
name: mcp-cli
description: Use when interacting with external tools, APIs, or data sources through MCP servers. Covers discovery, schema inspection, and tool invocation.
invocation: agent
---
...
1. Discover: `mcp-cli` to see available servers.
2. Explore: `mcp-cli info <server>` to see tools with parameters.
3. Inspect: `mcp-cli info <server> <tool>` to get full JSON schema.
4. Execute: `mcp-cli call <server> <tool> '<json>'` to run with arguments.
