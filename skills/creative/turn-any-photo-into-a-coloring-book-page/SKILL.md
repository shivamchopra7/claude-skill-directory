...
| ---: | ---: | --- | --- |
| 1 | Yes | Add the photo you want to convert into a coloring page  | `--image-input-url <url>` or `--image-input-base64 <base64>` or `--image-input-path <absolute path to image>` |
...
- Required images:
  - **Exactly 1** images: image 1 (Add the photo you want to convert into a coloring page ). Images must be provided in this order.
...
If the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (Add the photo you want to convert into a coloring page )"**.
