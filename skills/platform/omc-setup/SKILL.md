...
2. **Local Configuration (`--local`)**: Configure project-specific settings (.gemini-cli/GEMINI.md)
3. **Global Configuration (`--global`)**: Configure global settings (~/.gemini-cli/GEMINI.md)
...
**Options:**
1. **Local (this project)** - Creates `.gemini-cli/GEMINI.md` in current project directory. Best for project-specific configurations.
2. **Global (all projects)** - Creates `~/.gemini-cli/GEMINI.md` for all Gemini CLI sessions. Best for consistent behavior everywhere.
...
https://raw.githubusercontent.com/Yeachan-Heo/oh-my-gemini/main/docs/GEMINI.md
...
- GEMINI.md: Updated with latest configuration from GitHub at ./.gemini-cli/GEMINI.md
- Backup: Previous GEMINI.md backed up to `.gemini-cli/GEMINI.md.backup.YYYY-MM-DD` (if existed)
...
- GEMINI.md: Updated with latest configuration from GitHub at ~/.gemini-cli/GEMINI.md
- Backup: Previous GEMINI.md backed up to `~/.gemini-cli/GEMINI.md.backup.YYYY-MM-DD` (if existed)
- Scope: **GLOBAL** - applies to all Gemini CLI sessions
- Hooks: Provided by plugin (no manual installation needed)
- Agents: 28+ available (base + tiered variants)
- Model routing: Haiku/Sonnet/Opus based on task complexity
...
This will:
1. Install the HUD wrapper script to `~/.gemini-cli/hud/omc-hud.mjs`
2. Configure `statusLine` in `~/.gemini-cli/settings.json`
...
- Use `/oh-my-gemini:help` for guidance
- Use `/oh-my-gemini:doctor` for diagnostics
...
**Options:**
1. **Built-in Tasks (default)** - Use Gemini CLI's native TaskCreate/TodoWrite. Tasks are session-only.
...
- `/oh-my-gemini:omc-setup --local` to update project config
- `/oh-my-gemini:omc-setup --global` to update global config
