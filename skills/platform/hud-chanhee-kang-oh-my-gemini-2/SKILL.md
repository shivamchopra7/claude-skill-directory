...
role: config-writer  # DOCUMENTATION ONLY - This skill writes to ~/.gemini-cli/ paths
scope: ~/.gemini-cli/**  # DOCUMENTATION ONLY - Allowed write scope
---
...
1. Check if `~/.gemini-cli/hud/omc-hud.mjs` exists
2. Check if `statusLine` is configured in `~/.gemini-cli/settings.json`
3. If missing, create the HUD wrapper script and configure settings
4. Report status and prompt to restart Gemini CLI if changes were made

**IMPORTANT**: If the argument is `setup` OR if the HUD script doesn't exist at `~/.gemini-cli/hud/omc-hud.mjs`, you MUST create the HUD files directly using the instructions below.

### Setup Instructions (Run These Commands)

**Step 1:** Check if setup is needed:
```bash
ls ~/.gemini-cli/hud/omc-hud.mjs 2>/dev/null && echo "EXISTS" || echo "MISSING"
```

**Step 2:** Check if the plugin is built (CRITICAL - common issue!):
```bash
# Find the latest version and check if dist/hud/index.js exists
PLUGIN_VERSION=$(ls ~/.gemini-cli/plugins/cache/omc/oh-my-gemini/ 2>/dev/null | sort -V | tail -1)
if [ -n "$PLUGIN_VERSION" ]; then
  ls ~/.gemini-cli/plugins/cache/omc/oh-my-gemini/$PLUGIN_VERSION/dist/hud/index.js 2>/dev/null && echo "BUILT" || echo "NOT_BUILT"
fi
```
...
```bash
cd ~/.gemini-cli/plugins/cache/omc/oh-my-gemini/$PLUGIN_VERSION && npm install
```
...
```bash
mkdir -p ~/.gemini-cli/hud
```

Then, use the Write tool to create `~/.gemini-cli/hud/omc-hud.mjs` with this exact content:
...
async function main() {
  ...
  // 1. Try plugin cache first (marketplace: omc, plugin: oh-my-gemini)
  const pluginCacheBase = join(home, ".gemini-cli/plugins/cache/omc/oh-my-gemini");
  ...
  if (pluginCacheDir) {
    ...
    console.log("[OMC] Plugin not found. Run: /oh-my-gemini:omc-setup");
  }
}
...
```bash
chmod +x ~/.gemini-cli/hud/omc-hud.mjs
```

**Step 4:** Update settings.json to use the HUD:

Read `~/.gemini-cli/settings.json`, then update/add the `statusLine` field.
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node /home/username/.gemini-cli/hud/omc-hud.mjs"
  }
}
```
...
```bash
rm -f ~/.gemini-cli/hud/sisyphus-hud.mjs 2>/dev/null
```

**Step 6:** Tell the user to restart Gemini CLI for changes to take effect.
...
HUD config is stored at: `~/.gemini-cli/.omc/hud-config.json`
...
2. Restart Gemini CLI after setup completes
3. If still not working, run `/oh-my-gemini:doctor` for full diagnostics

Manual verification:
- HUD script: `~/.gemini-cli/hud/omc-hud.mjs`
- Settings: `~/.gemini-cli/settings.json` should have `statusLine` configured
