...
Prompt the user to choose which items to address (multi-select).
...
2. **Plan the fix:**
   ...
   - Complex (architectural): Delegate to an architect agent (see `agents/code-architect.md` for instructions) with context: the item title, severity, description, the relevant report section text, and any files or components mentioned. The agent designs the fix and returns a proposal.
   - Complex (needs investigation): This step uses the **code-explorer** agent from the **deep-analysis** skill (see `../deep-analysis/agents/code-explorer.md`) with context: the item title, description, suspected files/components, and what needs investigation. The agent explores and returns findings for you to formulate a fix proposal.
   - If an agent delegation fails, fall back to direct investigation using file reading and searching, and propose a simpler fix based on available information.
...
- **`agents/code-architect.md`** — Designs implementation blueprints for fixing complex architectural issues identified in the analysis report.

This skill also cross-references agents from the **deep-analysis** skill:

- **`../deep-analysis/agents/code-explorer.md`** — Used in Step 3c for investigating complex items that need codebase exploration before a fix can be proposed.
