...
1. **Determine analysis context:**
   ...
   - If no inputs, set context to "general codebase understanding"
...
3. **Run deep-analysis workflow:**
   - Refer to the **deep-analysis** skill for multi-agent codebase exploration and synthesis.
...
1. **Load diagram guidance:**
   - Refer to the **technical-diagrams** skill for Mermaid diagram syntax, styling rules, and best practices.
...
The following reference provides the standard structure for codebase analysis reports:
...
**Executive Summary:** Lead with the most important finding, not a generic overview. Keep to 2-3 sentences maximum. Include at least one actionable insight.

**Critical Files:** Limit to 5-10 files -- these should be the files someone must understand. Include both the "what" (purpose) and "why" (relevance to analysis context). File Details should cover exports, logic, and connections.

**Patterns & Conventions:** Only include patterns that are consistently applied (not one-off occurrences). Note deviations from patterns -- these are often more interesting than the patterns themselves.

**Relationship Map:** Focus on the most important connections, not an exhaustive dependency graph. Use directional language (calls, depends on, triggers, reads from). Highlight any circular dependencies or unexpected couplings. Include 2-3 levels of dependency depth. Use Mermaid flowcharts with `classDef` and `color:#000`. Cap at 15-20 connections.

**Challenges & Risks:** Rate severity based on likelihood and impact combined. Include specific details, not vague warnings. Focus on challenges relevant to the analysis context.
...
The following reference provides the standard format for presenting and processing actionable items:
...
**From Challenges & Risks Table:** Use the Severity column value directly (High, Medium, or Low). Title comes from the Challenge column. Description comes from the Impact column.

**From Recommendations Section:** Each recommendation in the report should explicitly cite which challenge it addresses. Use this citation to inherit severity:
...
**From Other Findings:** Default to Low unless the finding explicitly describes a critical issue. Only include findings that have a concrete, implementable fix.

### Complexity Assessment Criteria

**Simple (No agent needed):** Single file change. Clear, localized fix (rename, add validation, fix import, update config). No architectural impact. Change is self-contained.

**Complex -- Architectural (delegate to code-architect):** Requires refactoring across multiple files. Introduces or changes a pattern. Affects system architecture. Requires design decisions.

**Complex -- Investigation Needed (delegate to code-explorer):** Root cause is unclear or needs tracing. Multiple potential locations for the fix. Requires understanding current behavior before proposing changes.
...
- **Deduplication:** Merge items that target the same file/component, have significant keyword overlap, or where one is a superset of another. Keep the higher severity and merge descriptions.
...
- Maximum 3 revision cycles per item when user selects "Modify". After 3 cycles, present final proposal with Apply or Skip only.

Present items sorted High to Medium to Low. Prompt the user to select which items to address (multiple selections allowed). If no items selected, skip to Step 4.
...
2. **Plan the fix:**
   ...
   - Complex (architectural): Delegate to the **code-architect** agent with context: the item title, severity, description, the relevant report section text, and any files or components mentioned. The agent designs the fix and returns a proposal.
   - Complex (needs investigation): Delegate to the **code-explorer** agent with context: the item title, description, suspected files/components, and what needs investigation. The agent explores and returns findings for you to formulate a fix proposal.
   - If agent delegation fails, fall back to direct investigation using file reading and searching, and propose a simpler fix based on available information.
...
**What this component does:** Orchestrates end-to-end codebase analysis: runs deep-analysis for exploration, presents a structured report, and offers interactive post-analysis actions including saving reports, updating project docs, and addressing actionable insights.

**Capabilities needed:**
- File operations (read, write, edit, search by pattern, search by content)
- Shell execution (for session/cache management)
- Sub-agent delegation (via deep-analysis, plus code-architect and code-explorer for actionable insights)
- User interaction (multi-select prompts, approval flows, file path confirmation)

**Adaptation guidance:**
- Phase 1 delegates to deep-analysis which handles all agent orchestration. Your platform needs to support the deep-analysis skill's delegation pattern.
- Phase 3 actionable insights may delegate to code-architect or code-explorer agents for complex fixes. If your platform cannot spawn agents on demand, fall back to direct investigation.
- The report template and actionable insights template are embedded above. No external file references needed.
- Multi-select prompts are used in Steps 1, 2c-1, and 3b. If your platform only supports single selection, present options sequentially.

**Configurable parameters:** None specific to this skill. Inherits deep-analysis settings for cache and checkpointing behavior.
