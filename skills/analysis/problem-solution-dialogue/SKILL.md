...
- 候选阶段（第 5 步）：只输出“对齐信息 3 行” + 2–3 个候选方案；不输出流程图或中间推理产物
- 定稿阶段（第 6 步）：只输出两块内容：1) D2“决策流程图”（将同目录 `decision-flow.d2` 原样放入代码块）；2) 2–3 个可选方案（标注已选/推荐）
...
### 溯因推理（第 2 步：内部推理，不对用户输出）

- 溯因：列 2–3 个假设（含 1 个低概率高风险）并排序；给最可能假设的最小验证动作
- 护栏：低概率高风险未证伪前不淘汰；写早期信号/护栏；前提被否定则回退更新

### 系统思考（第 3 步：内部推理，不对用户输出）

- 画出关键要素与因果链（谁影响谁，哪里有反馈回路 R/B）；用于检查“方案收益点”是否有路径支撑
- 标注 1–2 个最关键的杠杆点与副作用通道；用于约束方案边界与风险项

### 演绎推理（第 4 步：内部推理，不对用户输出）

- 列 3–7 条关键断言（每条都要可证伪），并给出对应验证方式（数据/实验/检查点）
- 反推边界条件：哪些前提一旦不成立就必须回退（与“红旗信号”一致）

### 候选方案（第 5 步：输出 2–3 个，让用户选）
...
终稿：收敛后按“输出约束”输出，并让用户选择下一步（第 7 步 / 回到第 5 步 / 到此结束）。
...
- 阻断红旗：必须回退到 `RF-x -> 1/2/5` 指定步骤；回退后只做 1 件事（问 1 题或给 1 个最小验证）然后停（例：回退补齐成功标准）
