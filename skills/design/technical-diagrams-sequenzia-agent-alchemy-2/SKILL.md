...
**For advanced sequence, class, state, or ER diagrams**, read the corresponding reference file from the `references/` directory.

---

## Quick Reference

Minimal copy-paste examples for simple diagrams. For complex use cases, consult the corresponding reference section or file.
...
**C4 diagrams** -- Always consult the C4 Diagrams section below. C4 uses a unique function-call syntax that differs significantly from other Mermaid diagrams.

---

## Flowcharts Reference

The following reference provides complete flowchart syntax for building complex diagrams:
...
The following reference provides complete C4 diagram syntax. C4 uses a unique function-call syntax that differs from other Mermaid diagrams.
...
**What this component does:** Provides comprehensive Mermaid diagram syntax, styling rules, and examples for generating technical visualizations. This is the most widely referenced skill in the core-tools package -- loaded by 8 skills/agents across 4 plugin groups.

**Capabilities needed:** None -- this is a passive reference skill with no tool requirements.

**Adaptation guidance:** This skill is purely informational content. It works on any platform that supports Mermaid rendering. The styling rules (especially `color:#000` for dark text) are universal best practices. Reference files for sequence, class, state, and ER diagrams are in the `references/` subdirectory for advanced use cases.
