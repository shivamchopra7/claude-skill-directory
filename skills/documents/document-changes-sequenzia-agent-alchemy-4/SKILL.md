...
- **If `$ARGUMENTS` is provided**: Convert to kebab-case (e.g., `"auth refactor"` becomes `auth-refactor`)
...
**If user selects "Custom path":** Follow up asking for the full file path.
...
3. If the write fails, report the error and offer the user an alternative path.
...
2. "Commit changes" -- Suggest using a git commit workflow to commit outstanding changes
