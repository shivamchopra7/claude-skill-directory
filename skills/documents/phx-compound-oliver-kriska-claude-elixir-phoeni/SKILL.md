...
description: Capture solved problems as searchable solution docs. Use after fixing
  bugs, when "that worked", or after successful /skill:phx-review or /skill:phx-investigate.
---
...
```
/skill:phx-compound Fixed N+1 query in user listing
/skill:phx-compound Resolved LiveView timeout in dashboard
/skill:phx-compound   # Auto-detects from recent session context
```
...
`/skill:phx-permissions` to allow `.claude/solutions/` writes for next time.
...
"the fix was" — suggest `/skill:phx-compound`.

### Supply-chain finding auto-feed (Phase 3)

When `/skill:phx-deps-audit` produces a BLOCK-severity finding that the
...
> `/skill:phx-compound` to capture this for future audits?
...
`/skill:phx-deps-audit` grep `solutions/supply-chain/` for snippet
...
```text
/skill:phx-review → Complete → /skill:phx-compound  ← YOU ARE HERE
                              │
                 .claude/solutions/{category}/{fix}.md
                              │
              /skill:phx-investigate and /skill:phx-plan search here
```
