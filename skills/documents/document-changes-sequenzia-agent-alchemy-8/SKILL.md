...
- **File system access**: Read file system to check paths, write the report file
- **Shell execution**: Run git commands to gather change data and metadata
- **User interaction**: Prompt the user for report location and next steps

### Adaptation Guidance

- This skill depends entirely on git for change data. It will not work in non-git environments.
- The report template can be customized by modifying the template section above.
- No parallel delegation is required — this skill executes sequentially.
