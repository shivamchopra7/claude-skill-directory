...
5. **Documentation Generation** -- Delegate to docs-writer agents to generate content
...
1. Use the MkDocs Configuration Template below to scaffold the config
...
2. Delegate to 1 **code-explorer** agent (from the core-tools package) to analyze the changed files:
   ```
...
- Search file contents for broken cross-references between pages
