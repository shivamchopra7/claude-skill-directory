...
- Optional scope or description for the report (e.g., `"auth refactor"`, `"add OAuth2 support"`). Used to name the output file and populate the report's scope field. If not provided, scope is inferred from commit messages or changed file paths.
...
- **If inputs include a scope**: Convert to kebab-case (e.g., `"auth refactor"` -> `auth-refactor`)
- **If no scope provided**: Infer from the most common theme in commit messages or changed file paths (e.g., `api-bug-fixes`, `add-oauth2-support`, `ui-component-updates`)
...
**Scope**: {from inputs, or inferred from changes}
...
{List of staged files, or "No staged changes."}

### Unstaged Changes

{List of unstaged modifications, or "No unstaged changes."}

### Untracked Files

{List of untracked files, or "No untracked files."}
...
1. If the parent directory does not exist, create it:
   ...
   mkdir -p {parent_directory}
   ```
2. Write the report file at the chosen path.
3. If the write fails, report the error and prompt the user to offer an alternative path.
...
Options:
1. "Review report" -- Read the report back to the user
2. "Commit changes" -- Suggest committing outstanding changes
...
| Write fails | Offer alternative path via user prompt |
...
**What this component does:** Generates a structured markdown report documenting all codebase changes from a working session, including files changed, commit history, git status, and human-readable descriptions.

**Capabilities needed:**
- File read/write operations
- Shell command execution (git commands, mkdir, date)
- User interaction (prompts for report location)
- Search for files matching patterns (optional, for path validation)

**Adaptation guidance:**
- This skill is entirely git-based -- it requires a git repository to function
- The report template can be customized by modifying the template in Step 4
- The recommended output path (`internal/reports/`) is a convention that can be changed
- Works independently with no external skill or agent dependencies
