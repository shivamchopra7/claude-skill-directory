...
```
/skill:phx-recall how did we fix the LiveView form that saved silently?
/skill:phx-recall what did we decide about the billing context boundaries?
/skill:phx-recall which library did we pick for rate limiting and why?
```
...
NOT already in `.claude/solutions/`, offer `/skill:phx-compound` so the next
recall stops at Layer 1.

## Integration

```text
"have we done this before?" → /skill:phx-recall
   Layer 1 .claude/solutions/ ──hit──► answer + cite
   Layer 2 git log --grep/-S  ──hit──► answer + cite
   Layer 3 ccrider sessions (gated) ─► answer + cite → offer /skill:phx-compound
```
