---
name: update-reqs
description: Add or update functional requirement documents in docs/requirements/. Use when user says "update reqs", "update requirements", "sync reqs", or wants to maintain requirement documents that describe what the software must do.
hooks:
  PreToolUse:
    - matcher: "*"
      hooks:
...
Requirement files live in `docs/requirements/`. Create files as needed based on the project's components and domain areas. Organize by the natural boundaries of the system — one file per major subsystem, domain, or integration surface.
...
- Functional areas group related requirements under a logical heading
...
Launch parallel Explore subagents to understand what the software currently does. Focus on observable behavior and contracts, not implementation structure. For each component being updated, explore: what inputs it accepts, what outputs it produces, what invariants it upholds, and what contracts it exposes to callers.
...
- **`/update-specs`** — Maintains functional specification documents (more detailed than requirements)
- **`/make-req`** — Decomposes a task or plan into requirements (this skill maintains the living docs)
