...
## Integration Notes
**What this component does:** Provides comprehensive Keep a Changelog format guidelines, entry writing best practices, category definitions, and examples of well-written vs poorly-written entries.
**Capabilities needed:** None (pure knowledge/reference skill).
**Adaptation guidance:** This is a passive knowledge skill with no tool or agent dependencies. It can be loaded by any workflow that produces changelog entries. The inline examples section (formerly a separate reference file) provides concrete good/bad examples for each category.
