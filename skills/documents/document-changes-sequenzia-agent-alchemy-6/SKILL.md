...
Build the markdown report with the following structure, then write it.
...
3. If the write fails, report the error and offer an alternative path.
...
## Integration Notes
**What this component does:** Generates a structured markdown report documenting all codebase changes in the current session, including files changed, commit history, staged/unstaged status, and a human-readable summary.
**Capabilities needed:** Shell execution (git commands), file writing, user interaction (for path selection and next steps).
**Adaptation guidance:** This skill relies heavily on git CLI commands for change detection. The report template and git commands are platform-agnostic. Adapt the user interaction prompts to your platform's dialog system.
