...
3. **Group or prefix**: Is this skill part of a group (e.g. workflow, jira, git)? If yes, ask for the group name and use it as a prefix in the skill name (e.g. `workflow-create-ticket`, `jira-start-ticket`).
...
- "Is this skill part of a group?" with options like ["Yes", "No"]. If Yes, ask "What's the group name?" and use it as a prefix (lowercase, hyphens): prefix-skill-name.
...
|-------|--------------|---------|
| `name` | Max 64 chars, lowercase letters/numbers/hyphens only. Use `prefix-skill-name` when part of a group (e.g. `workflow-create-ticket`). | Unique identifier for the skill |
| `description` | Max 1024 chars, non-empty | Helps agent decide when to apply the skill |

**Naming with groups**: If the skill belongs to a group, ask for the group name and format the skill name as `group-skill-name` (all lowercase, hyphens only). Examples: `workflow-create-ticket`, `jira-start-ticket`, `git-prepare-commit`.
...
3. Whether the skill is part of a group—if yes, the group name to use as prefix (e.g. workflow, jira)
4. Trigger scenarios
5. Any specific requirements or constraints
6. Existing examples or patterns to follow

If you have access to the AskQuestion tool, use it for efficient structured gathering. Otherwise, ask conversationally.

### Phase 2: Design

1. Draft the skill name (lowercase, hyphens, max 64 chars). If the skill is part of a group, use the prefix: `group-name-skill-name` (e.g. `workflow-create-ticket`, `jira-start-ticket`).
