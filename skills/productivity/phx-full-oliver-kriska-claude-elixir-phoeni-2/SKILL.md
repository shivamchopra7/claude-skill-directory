...
description: Use for large features spanning multiple contexts or autonomous end-to-end
  ...
  agents. NOT for executing an existing plan file — use /skill:phx-work for that.
---
...
/skill:phx-full Real-time notification system with Phoenix PubSub
/skill:phx-full Background job processing for email campaigns --max-cycles 5
/skill:phx-full Add magic-link auth --codex
```

**Wrong input guard**: if the argument is a path to an existing plan file
(`.claude/plans/*/plan.md`), do NOT re-plan it. Say so and run `/skill:phx-work {path}`
instead — the plan phase already happened.

## Flags

- **`--codex`** — Codex CLI joins the review panel on **every** review
  cycle, matching `/skill:phx-review --codex`.
...
│  Auto-suggest: /skill:phx-document → /skill:phx-learn-from-fix                       │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
Suggest `/skill:phx-document` for docs and `/skill:phx-learn-from-fix` for quick pattern capture.
...
```text
/skill:phx-full = /skill:phx-plan → /skill:phx-work → /skill:phx-verify → /skill:phx-review → (fix → /skill:phx-verify) → /skill:phx-compound
```
