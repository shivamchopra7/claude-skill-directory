---
name: af-plan-work-cycles
description: Plan weekly work cycles and manage team capacity budgets. Use when prioritizing backlogs, scheduling issues into Linear cycles, or balancing workload across team members.
...
parent: ../README.md
related:
  - ../af-estimate-effort/SKILL.md
  - ../af-manage-work-state/SKILL.md
  - ../af-query-linear-api/SKILL.md
  - ../af-discover-scope/SKILL.md
  - ../af-refine-specifications/SKILL.md
---
...
The estimation skill (`af-estimate-effort`) defines two tiers:
...
- Issues with no estimate → estimate first using `af-estimate-effort`, then schedule
...
3. **No estimate?** Load `af-estimate-effort` and run a Discovery Estimate (quick heuristic)
...
| **Needs estimation** | No estimate (null) | Estimate using af-estimate-effort heuristics |
...
**Estimation skill** (`af-estimate-effort`):
...
**Process skills:**
- `af-discover-scope` — Features should get Discovery Estimates during Phase 4
- `af-refine-specifications` — Features should get Refined Estimates before approval
- `af-deliver-features` — Story points must be set before starting implementation
