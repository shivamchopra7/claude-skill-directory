...
description: Create hierarchical project plans optimized for solo agentic development. Use when planning projects, phases, or tasks that the AI agent will execute. Produces agent-executable plans with verification criteria, not enterprise documentation. Handles briefs, roadmaps, phase plans, and context handoffs.
---
...
- **I am Gemini CLI** → Follow [GEMINI] instructions

Instructions without markers apply to all agents.
</agent_identification>

<cli_paths>
**Substitute paths based on your CLI:**

| Resource | [CLAUDE] / [COPILOT] | [GEMINI] |
|----------|----------------------|----------|
| Skills | `~/.claude/skills/` | `~/.gemini/skills/` |

Examples in this skill use Claude paths. Substitute your CLI's path.
</cli_paths>

<essential_principles>

<principle name="solo_developer_plus_agent">
You are planning for ONE person (the user) and ONE implementer (you).
No teams. No stakeholders. No ceremonies. No coordination overhead.
The user is the visionary/product owner. You are the builder.
</principle>
...
**Critical insight:** LLMs don't degrade at 80% - they degrade at ~40-50% when context mounts and they enter "completion mode." By 80%, quality has already crashed.
...
**You automate everything that has a CLI or API.** Checkpoints are for verification and decisions, not manual work.

**Checkpoint types:**
- `checkpoint:human-verify` - Human confirms your work (aesthetic preferences, brand alignment)
...
**Critical rule:** If you CAN do it via CLI/API/tool, you MUST do it. Never ask human to:
...
- Verify visual/UI changes:
  - [CLAUDE] Built-in (`claude --chrome`), tests, or browser automation MCP
  - [COPILOT] [GEMINI] Browser automation MCP (Chrome DevTools, Playwright, Puppeteer)

**Protocol:** You automate work → reach checkpoint:human-verify → present what was done → wait for confirmation → resume
...
| "execute", "run", "do it", "build it", 2 (has structure) | **EXIT SKILL** → [CLAUDE] `/run-plan <path>` [COPILOT] [GEMINI] `run_skill("kitt-run-plan", "<path>")` |
...
**Critical:** Plan execution should NOT invoke this skill. [CLAUDE] Use `/run-plan` [COPILOT] [GEMINI] Use `run_skill("kitt-run-plan")` for context efficiency (skill loads ~20k tokens, run-plan loads ~5-7k).
...
FINDINGS.md       → Research output (if research done)
    ↓
PLAN.md           → THE PROMPT (you execute this)
    ↓
...
- Plan execution uses [CLAUDE] `/run-plan` [COPILOT] [GEMINI] `run_skill("kitt-run-plan")` (not skill invocation)
</success_criteria>
