...
3. **Spawns** up to 5 parallel workers (Gemini CLI limit)
...
```
/oh-my-gemini:ultrapilot <your task>
/oh-my-gemini:up "Build a full-stack todo app"
/oh-my-gemini:ultrapilot Refactor the entire backend
```
...
**Max Workers:** 5 (Gemini CLI limit)
...
**Settings Explained:**
- `maxWorkers` - Max parallel workers (5 is Gemini CLI limit)
...
```
/oh-my-gemini:ultrapilot resume
```
...
```
/oh-my-gemini:ultrapilot Build a todo app with React frontend, Express backend, and PostgreSQL database
```
...
```
/oh-my-gemini:up Refactor all services to use dependency injection
```
...
```
/oh-my-gemini:ultrapilot Generate tests for all untested modules
```
...
Then run:
```
/oh-my-gemini:ultrapilot --custom-decomposition
```
...
**Planned for v4.2:**
- Multi-machine distribution (if Gemini CLI supports)
