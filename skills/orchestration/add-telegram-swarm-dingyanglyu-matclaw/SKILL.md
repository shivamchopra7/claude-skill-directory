...
1. *Share progress in the group* via `mcp__matclaw__send_message` with a `sender` parameter matching their exact role/character name (e.g., `sender: "Marine Biologist"` or `sender: "Alexander Hamilton"`). This makes their messages appear from a dedicated bot in the Telegram group.
...
Also add `TELEGRAM_BOT_POOL` to the launchd plist (`~/Library/LaunchAgents/com.matclaw.plist`) in the `EnvironmentVariables` dict if using launchd.
...
>
> Check logs: `tail -f logs/matclaw.log | grep -i pool`
...
2. Check pool initialized: `grep "Pool bot" logs/matclaw.log`
...
8. Rebuild: `npm run build && ./container/build.sh && launchctl unload ~/Library/LaunchAgents/com.matclaw.plist && launchctl load ~/Library/LaunchAgents/com.matclaw.plist` (macOS) or `npm run build && ./container/build.sh && systemctl --user restart matclaw` (Linux)
