---
name: cohalo-create-agent
description: >
  Create new agents in CoHalo with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# CoHalo Create Agent Skill
...
2. Discover available adapter configuration docs for this CoHalo instance.
...
```sh
curl -sS "$COHALO_API_URL/llms/agent-configuration/claude_local.txt" \
  -H "Authorization: Bearer $COHALO_API_KEY"
```
...
```sh
curl -sS -X POST "$COHALO_API_URL/api/companies/$COHALO_COMPANY_ID/agent-hires" \
  -H "Authorization: Bearer $COHALO_API_KEY" \
...
- when the board approves, you will be woken with `COHALO_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$COHALO_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $COHALO_API_KEY"
