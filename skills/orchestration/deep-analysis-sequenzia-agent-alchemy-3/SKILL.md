...
4. **Assign exploration tasks (with status guard):**
   ...
   5. Communicate the task details to the explorer:
...
2. Communicate the exploration context and recon findings to the synthesizer:
...
- If `.agents/agent-alchemy.local.md` exists but is malformed or the `deep-analysis` section is unparseable: warn the user ("Settings file found but could not parse deep-analysis settings — using defaults") and proceed with default approval values.
...
| **Phase 5** | Load all explorer findings from session dir. Start a fresh synthesizer and launch synthesis with the persisted findings. |
...
- Session checkpointing writes to `.agents/sessions/` — adapt the path to the target harness's session storage.
