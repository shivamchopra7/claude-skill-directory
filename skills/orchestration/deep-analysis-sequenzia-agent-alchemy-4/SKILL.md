...
5. **Parse session settings** (also under the `deep-analysis` section):
   ...
   - `enable-progress-indicators`: Whether to display progress messages. Default: `true`.
...
3. **If cache is valid:**
   ...
   - **Direct invocation:** Prompt the user:
...
2. **Rapid codebase reconnaissance:**
   ...
   - **Key documentation:** Read `README.md`, `CLAUDE.md`, `AGENTS.md`, or similar docs if they exist for project context
...
1. **Present the team plan** to the user (output the plan from Phase 1 Step 4), then prompt the user:
...
**Goal:** Create the agent team, spawn agents, create tasks, and assign work using the approved plan.

> If `ENABLE_PROGRESS = true`: Display "**[Phase 3/6] Team Assembly** — Creating team and spawning agents..."

1. **Create the team:**
   - Create a team named `deep-analysis-{timestamp}` (e.g., `deep-analysis-1707300000`)
   - Description: "Deep analysis of [analysis context]"

2. **Spawn teammates:**
   Based on the approved plan, spawn the following agents:

   - **N explorers** (one per focus area):
     - Named: `explorer-1`, `explorer-2`, ... `explorer-N`
     - Each explorer follows the **code-explorer** agent specification (see `agents/code-explorer.md`)
   ...
   - **1 synthesizer**:
     - Named: `synthesizer`
     - Follows the **code-synthesizer** agent specification (see `agents/code-synthesizer.md`)
...
3. **Create tasks:**
   ...
   - **Synthesis Task:** Subject: "Synthesize and evaluate exploration findings", Description: "Merge and synthesize findings from all exploration tasks into a unified analysis. Investigate gaps using shell commands (git history, dependency trees). Evaluate completeness before finalizing."
     - Set the synthesis task as blocked by all exploration task IDs
...
4. **Shutdown teammates:**
   ...
   - Send shutdown to each explorer
   - Send shutdown to the synthesizer
...
- If maximum modification cycles (3) or regeneration cycles (2) are reached without approval, prompt the user:
...
**What this component does:** Orchestrates a multi-agent deep exploration and synthesis workflow. A lead agent performs reconnaissance, plans focus areas, spawns N explorer agents and 1 synthesizer agent, monitors exploration, then triggers synthesis of findings into a unified analysis.

**Capabilities needed:** Agent/team creation and management, task creation and assignment, inter-agent messaging, file reading, file search (by pattern and content), shell command execution, user prompting.

**Adaptation guidance:** The core orchestration pattern is hub-and-spoke: a lead creates a plan, spawns workers, assigns tasks, monitors completion, then launches synthesis. Map the team/task/messaging primitives to the host platform's agent coordination system. The explorer and synthesizer agent specifications are in `agents/code-explorer.md` and `agents/code-synthesizer.md` respectively.

**Configurable parameters:**
- `deep-analysis.direct-invocation-approval` (default: true) — require plan approval on direct invocation
- `deep-analysis.invocation-by-skill-approval` (default: false) — require plan approval when loaded by another skill
- `deep-analysis.cache-ttl-hours` (default: 24) — hours before exploration cache expires; 0 disables caching
- `deep-analysis.enable-checkpointing` (default: true) — write session checkpoints at phase boundaries
- `deep-analysis.enable-progress-indicators` (default: true) — display progress messages during execution

**Sub-agent capabilities:**
- **code-explorer** (N instances, one per focus area): File reading, file search, content search, shell commands, inter-agent messaging. Draws on knowledge from the **project-conventions** and **language-patterns** skills.
- **code-synthesizer** (1 instance): File reading, file search, content search, shell commands (including git), inter-agent messaging. Draws on knowledge from the **project-conventions**, **language-patterns**, and **technical-diagrams** skills.
