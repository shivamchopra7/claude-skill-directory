...
if [[ -f .omc/state/autopilot-state.json ]]; then
  ...
  echo "Run /oh-my-gemini:autopilot to resume."
fi
```
...
if [[ -f .omc/state/ultrawork-state.json ]]; then
  ...
  if [[ "$LINKED" == "true" ]]; then
    echo "Ultrawork is linked to Ralph. Use /oh-my-gemini:cancel to cancel both."
    exit 1
  fi
...
if [[ -f .omc/state/autopilot-state.json ]]; then
  ...
  if [[ "$AUTOPILOT_ACTIVE" == "true" ]]; then
    ...
    echo "Progress preserved for resume. Run /oh-my-gemini:autopilot to continue."
