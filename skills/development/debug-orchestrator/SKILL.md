...
description: Systematic 5-step debugging ladder for broken story executions. Use when a story is at executing (broken) stage.
---

# Debug Orchestrator

Fix broken story executions using a systematic 5-step debugging ladder. Each step applies a specific debugging technique with increasing sophistication.

## When to Use

Use this skill when:
- A story is at `executing (broken)` stage
- Execution failed with `status: partial` or `status: failed`
- Tests are failing after an execution attempt
...
Step 5: systematic-debugging → Full 4-phase framework
   ↓
Escalate → executing (escalated)
```

Each step is attempted in order. If a step fixes the issue, execution is retried. If all 5 steps fail, the story is escalated for human review.
...
**Exit condition:** All pattern tests pass → retry execution
...
**Exit condition:** Root cause fixed → retry execution
...
**Exit condition:** API updated to current pattern → retry execution
...
```markdown
## Context
Story [ID] execution failed at [stage].
...
**Exit condition:** Opus fix applied → retry execution
...
**Exit condition:** Fix verified → retry execution
