...
forked and modified from: https://clawhub.ai/MahmoudAdelbghany/ffmpeg-video-editor
---
...
- Balanced compression (CRF 28):
  ```bash
  ffmpeg -y -hide_banner -i "video.mp4" -c:v libx265 -tag:v hvc1 -crf 28 -preset medium -c:a aac -b:a 128k "video_compressed.mp4"
  ```
- High compression/smaller file (CRF 32):
  ```bash
  ffmpeg -y -hide_banner -i "video.mp4" -c:v libx265 -tag:v hvc1 -crf 32 -preset fast -c:a aac -b:a 96k "video_small.mp4"
  ```
- High quality (CRF 20):
  ```bash
