# Sling Dispatch — Pipeline Implementation

Implementation reference for `sling-dispatch`. Loaded on demand; the activation prose lives in `../SKILL.md`.

## Stage 1 — Fetch

Resolve the bead ID to a concrete work item from state. The bead must exist and be in `open` status. If already `hooked`, `in_progress`, or `done`, the dispatch is a no-op (idempotency guarantee).
