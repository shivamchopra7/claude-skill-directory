...
def model_definition(mo, pm, X, y):
    ...
    return mo.show_code(model)

@app.cell
def sampling(mo, pm, model):
    with model:
        trace = pm.sample(1000)
    # Position: "above" or "below" (default)
    return mo.show_code(trace, position="above")
```

Key points:
- `mo.show_code(output)` - shows code below output (default)
- `mo.show_code(output, position="above")` - shows code above output
- In displayed code, `mo.show_code(...)` is replaced with `...`
