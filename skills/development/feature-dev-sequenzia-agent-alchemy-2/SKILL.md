...
dependencies:
  ...
  - code-quality
  - changelog-format
  - code-architect agent (core-tools)
  - code-reviewer agent
---
...
- A feature description provided by the user
...
1. **Run deep-analysis workflow:**
   - Refer to the **deep-analysis** skill (from the core-tools package) for structured codebase exploration
...
2. **Delegate to code-architect agents:**

   Delegate to 2-3 **code-architect** agents (from the core-tools package) with different approaches:
   ```
...
1. **Require explicit approval:**
   Prompt the user: "Ready to begin implementation of [feature] using [chosen approach]?"
...
5. Proceed to Phase 6 once implementation is complete.
...
2. **Delegate to code-reviewer agents:**

   Delegate to 3 **code-reviewer** agents with different focuses:
   ```
...
7. Proceed to Phase 7 once the review is resolved.
...
2. Prompt the user for how to proceed:
   - Retry the phase
   - Skip to next phase
   - Abort the workflow
...
**What this component does:** Orchestrates a complete feature development lifecycle from discovery through implementation and review, coordinating multiple specialized agents for architecture design and code review.

**Capabilities needed:**
- File read/write/edit operations
- Shell command execution (for running tests)
- User interaction (prompts, confirmations, multiple-choice)
- Sub-agent delegation (code-architect, code-reviewer, deep-analysis workers)
- Search for files matching patterns
- Search file contents

**Adaptation guidance:**
- The 7-phase structure can be simplified for smaller features by merging Discovery and Clarifying Questions
- Agent delegation (Phase 4 architects, Phase 6 reviewers) can be replaced with sequential analysis if your platform does not support parallel agents
- Deep-analysis in Phase 2 requires the core-tools package; replace with your platform's codebase exploration if unavailable
- ADR generation in Phase 4 and CHANGELOG updates in Phase 7 are optional artifacts that can be skipped
