...
`/skill:phx-freeze [args]` — resolve `$ARGUMENTS` and run the matching Bash branch.

| Invocation | Effect |
|------------|--------|
| `/skill:phx-freeze` | Freeze ALL edits — read-only investigation mode |
| `/skill:phx-freeze lib/app_web priv/repo` | Allow edits only under these dirs |
| `/skill:phx-freeze status` | Show current lock state |
| `/skill:phx-freeze off` | Lift the lock (delete the sentinel) |

### Freeze all edits (investigation mode)

```bash
mkdir -p .claude && : > .claude/.freeze
echo "Freeze ON — all edits blocked. Lift with /skill:phx-freeze off"
```
...
2. **NEVER leave a freeze active across unrelated tasks** — it persists until
   `/skill:phx-freeze off`, including into later sessions. Clear it when the task ends.
...
- Pairs with `/skill:phx-investigate` (freeze all while root-causing) and `/skill:phx-work`
  (scope to the plan's dirs). The lock is advisory tooling, not a security
  boundary — anyone can run `/skill:phx-freeze off`.
