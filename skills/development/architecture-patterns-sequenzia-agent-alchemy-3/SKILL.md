...
Architecture visualizations use Mermaid syntax with `classDef` styling (`color:#000` for text readability). When creating architecture visualizations based on these patterns, follow consistent diagram styling conventions for dark text on nodes.

## Pattern Selection Guide

Choose patterns based on:
1. **Existing architecture** - Match what is already in use
...
1. **Match existing architecture** - Do not introduce new patterns unnecessarily
...
**What this component does:** Provides a reference catalog of architectural patterns (layered, MVC, repository, service layer, event-driven, CQRS, hexagonal, microservices) with selection guidance, Mermaid diagrams, and implementation tips.

**Capabilities needed:**
- None (reference-only skill -- provides knowledge, does not require tool access)

**Adaptation guidance:**
- This is a passive knowledge skill loaded by feature-dev during architecture design
- The Mermaid diagrams use a specific styling convention (dark text on light backgrounds) -- adapt if your rendering environment has different requirements
- Can be extended with additional patterns relevant to your domain
