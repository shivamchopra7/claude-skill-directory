...
- **code-reviewer** (`agents/code-reviewer.md`) -- Reviews code for correctness, security, and maintainability with confidence-scored findings. Used in Phase 6.
- **code-architect** (cross-reference: `../../core-tools/skills/codebase-analysis/agents/code-architect.md` from the core-tools package) -- Designs implementation blueprints. Used in Phase 4.
...
1. **Load skills for this phase:**
   - Refer to the **architecture-patterns** skill for design pattern guidance
   - Refer to the **language-patterns** skill (from the core-tools package) for language-specific conventions
   - Refer to the **technical-diagrams** skill (from the core-tools package) for styling rules on any Mermaid diagrams in architecture proposals
...
5. **Generate ADR artifact:**
   - Use the ADR template (see below) to create an ADR documenting:
...
2. **Update CHANGELOG.md:**
   - Use the changelog entry template (see below) for formatting guidance
...
- **code-reviewer** -- Launched in Phase 6 with different focus areas (correctness, security, maintainability). Each reviewer produces confidence-scored findings. Only issues with confidence >= 80 are reported.

**From the core-tools package (cross-reference):**
- **code-architect** -- Launched in Phase 4 to produce competing implementation blueprints. Each architect receives a different design philosophy (minimal, flexible, project-aligned).
- **deep-analysis** -- Used in Phase 2 for codebase exploration. Orchestrates its own explorer and synthesizer agents internally.
