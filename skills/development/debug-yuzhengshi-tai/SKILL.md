...
Authentication variables (`CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`) are extracted from `.env` and mounted as a file inside the container rather than passed via `-e`. This keeps credentials out of process listings.
...
**Docker mount syntax:**
- Both read-write and readonly use `-v`:
  ```bash
  # Readonly
