...
- Always read [skills/vitaecontext/wiki/vitaecontext.md](../vitaecontext/wiki/vitaecontext.md) before running any maintenance task so the agent understands the full project architecture before touching any file.
- If the root wiki entry is unavailable, read [skills/vitaecontext/SKILL.md](../vitaecontext/SKILL.md), [.assets/docs/architecture-map.md](../../.assets/docs/architecture-map.md), and [MAINTAINING.md](../../MAINTAINING.md) before proceeding. State that the root wiki entry was unavailable.
...
- `skills/vitaecontext-<module>/wiki/index.md` when source changes require a different conditional load map
- `skills/vitaecontext-<module>/SKILL.md` when source changes require different routing, load, or high-level operating rules
- `skills/vitaecontext-<module>/references/*.md`
...
- `llms.txt`
- `README.md`
- Provider adapter files under `providers/`
...
2. Read `skills/vitaecontext-<module>/wiki/knowledge.md` to understand current claims, confidence labels, `last_reviewed`, and `review_by`.
...
7. Inspect downstream module files that may need aligned updates: `hub/<module>/*.md`, `skills/vitaecontext-<module>/references/*.md`, `skills/vitaecontext-<module>/SKILL.md`, and `skills/vitaecontext-<module>/wiki/index.md`.
...
9. Identify forbidden files that still need separate follow-up outside this skill's write permissions, including README, CHANGELOG, provider wrappers, install behavior, or files outside the target module.
...
1. Every skill configured in `src/export-config.json` is represented in root routing and provider-capable command maps.
...
1. Read the architecture map, style guide, root runtime skill, export configuration, and VitaeGraph specification when applicable.
