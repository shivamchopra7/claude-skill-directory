...
`{{PLUGIN_ROOT}}/includes/smarts/core.md` — read it before Phase 3. The append-only decision-log
entry format is in `{{PLUGIN_ROOT}}/includes/smarts/decision-log-format.md` — read it before
writing a log line.
...
`{{PROJECT_DIR}}/.codearbiter/plans/`, then the project root, then `docs/`. MUST NOT
...
choice, confirm it back in one sentence, then append the decision to the log per `{{PLUGIN_ROOT}}/includes/smarts/decision-log-format.md`
...
(`{{PLUGIN_ROOT}}/agents/decision-challenger.md`) to stress-test an ADR. Optional, not forced.
