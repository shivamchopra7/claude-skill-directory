---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenPaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenPaw cron scheduling for periodic checks, or version status checks on a machine running OpenPaw (laptop, workstation, Pi, VPS).
---

# OpenPaw Host Hardening

## Overview

Assess and harden the host running OpenPaw, then align it to a user-defined risk tolerance without breaking access. Use OpenPaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenPaw changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenPaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenPaw can access sensitive data.
...
### 2) Run OpenPaw security audits (read-only)

As part of the default read-only checks, run `openpaw security audit --deep`. Only offer alternatives if the user requests them:

1. `openpaw security audit` (faster, non-probing)
2. `openpaw security audit --json` (structured output)

Offer to apply OpenPaw safe defaults (numbered):

1. `openpaw security audit --fix`

Be explicit that `--fix` only tightens OpenPaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenPaw version/update status (read-only)

As part of the default read-only checks, run `openpaw update status`.
...
- Credential hygiene notes (location of OpenPaw creds, prefer disk encryption)
...
After OpenPaw install or first hardening pass, run at least one baseline audit and version check:

- `openpaw security audit`
- `openpaw security audit --deep`
- `openpaw update status`

Ongoing monitoring is recommended. Use the OpenPaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openpaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openpaw update status`
...
Before creating, `openpaw cron list` and match on exact `name`. If found, `openpaw cron edit <id> ...`.
If not found, `openpaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openpaw update status` (preferred for source checkouts and channels)
2. `npm view openpaw version` (published npm version)

## OpenPaw command accuracy

Use only supported commands and flags:

- `openpaw security audit [--deep] [--fix] [--json]`
- `openpaw status` / `openpaw status --deep`
- `openpaw health --json`
- `openpaw update status`
- `openpaw cron add|list|runs|run`

Do not invent CLI flags or imply OpenPaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenPaw compaction:
