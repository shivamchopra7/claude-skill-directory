...
The CLI uses Chrome/Chromium via CDP directly. Install via `bun i -g agent-browser`, `brew install agent-browser`, or `cargo install agent-browser`. Run `agent-browser install` to download Chrome. Run `agent-browser upgrade` to update to the latest version.
...
**Requirements:** macOS with Xcode, Appium (`bun install -g appium && appium driver install xcuitest`)
