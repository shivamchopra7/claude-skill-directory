...
**Bug Categories:**
- **Off-by-One** -- Check `<` vs `<=`, 0-based vs 1-based, inclusive vs exclusive ranges
...
1. **Review the fix against code quality principles:**
...
## Integration Notes
**What this component does:** Provides a systematic, hypothesis-driven debugging workflow with triage-based routing (quick vs deep track), parallel agent investigation, regression testing, and project learning capture.
**Capabilities needed:** Shell execution (test runners, git commands), file reading/writing/editing, pattern search, sub-agent spawning (code-explorer from core-tools, bug-investigator), user interaction.
**Adaptation guidance:** The quick track is a single-agent workflow; the deep track requires spawning parallel sub-agents for exploration and hypothesis testing. Adapt agent spawning to your platform's sub-task mechanism. Language-specific debugging references are in `references/`.
