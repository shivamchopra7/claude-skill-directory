...
dependencies:
  - code-quality
  - project-learnings
  - code-explorer agent (core-tools)
...
- A bug description or error message, optionally with a `--deep` flag

Extract from the inputs and conversation context:
...
- Options: "Quick track" / "Deep track"
...
| TypeScript / JavaScript | See **references/typescript-debugging.md** for TS/JS-specific debugging techniques. |
| Other / Multiple | Use the General Debugging Reference below. |
...
3. **Check recent changes** -- run `git log --oneline -5 -- <file>` for the affected files
...
2. **Delegate to code-explorer agents:**

   Delegate to 2-3 **code-explorer** agents (from the core-tools package):
...
2. **Delegate to bug-investigator agents:**

   Delegate to 1-3 **bug-investigator** agents to test hypotheses in parallel:
...
4. **If stuck after 2 rounds of investigation:**
   ...
   - Prompt the user for additional context or direction:
...
These are read-only agents (from core-tools) that explore codebase areas. Give each a distinct focus area related to the bug. They report structured findings.

### Bug Investigators (Phase 3, deep track)

These are agents with shell access for running tests and git commands, but no write/edit capability -- they investigate and report evidence, they do not fix code. Give each a specific hypothesis to test.
...
3. Prompt the user for how to proceed:
   - "Retry this phase"
...
**What this component does:** Provides a systematic, hypothesis-driven debugging workflow that enforces investigation before fixes. Routes bugs to quick or deep investigation tracks based on complexity signals, coordinates exploration and investigation agents, and captures learnings.

**Capabilities needed:**
- File read/write/edit operations
- Shell command execution (for running tests, git commands)
- User interaction (prompts, confirmations, multiple-choice)
- Sub-agent delegation (code-explorer for deep track exploration, bug-investigator for hypothesis testing)
- Search for files matching patterns
- Search file contents

**Adaptation guidance:**
- The quick/deep track routing can be simplified to always use one track if your platform does not support parallel agents
- Bug-investigator agents (Phase 3 deep track) require shell access for running tests -- ensure agents have appropriate permissions
- Code-explorer agents (Phase 2 deep track) are read-only; they only need file reading and search capabilities
- The hypothesis journal format is the core value -- preserve it even if you simplify the agent coordination
