---
name: af-configure-test-frameworks
description: Configure test frameworks and analyse test coverage across Jest, RTL, and Playwright. Use when setting up test infrastructure, debugging test failures, or choosing testing strategies for a feature.
...
**Note:** GainInsight Standard projects should have paired test scripts configured. See `af-setup-project` skill for the full pattern (`test:integration` for CI/CD, `test:integration:local` for local dev).
