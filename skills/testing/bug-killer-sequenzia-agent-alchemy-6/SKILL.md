...
- **bug-description-or-error** — What's failing? Error messages, symptoms
- **--deep** (optional flag) — If present, skip triage and go directly to deep track (jump to Phase 2 deep track)

Extract from the input and conversation context:
...
- Prompt the user to choose: "Quick track (Recommended)" / "Deep track" — depending on your assessment
...
2. **Delegate to code exploration** (from the core-tools package):

   For each focus area, investigate:
   ...
   Launch explorations in parallel for independent focus areas.

3. **Synthesize exploration results:**
   - Collect findings from all explorations
   - Identify convergence (multiple areas pointing to same root cause)
...
2. **Delegate to bug investigation:**

   For each hypothesis, test it in parallel:

   Provide:
   - Bug context: description of the bug and error
   - Hypothesis to test: the specific hypothesis
   - Test plan with concrete steps (e.g., run this test, check git blame, trace data flow)
   - Request a report with verdict (confirmed/rejected/inconclusive), evidence, and recommendations

   Launch investigations in parallel when they test independent hypotheses.

3. **Evaluate results:**
   - Update hypothesis journal with each investigation's findings
...
4. **If stuck after 2 rounds of investigation:**
   ...
   - Prompt the user to choose: "Continue investigating", "Try a different angle", "Provide more context"
...
1. **Refer to the **code-quality** skill** for code quality principles and review the fix against them.
...
2. **Check for related issues:**
   ...
   - Prompt the user to choose: "Fix all related instances now?" / "Fix only the reported bug" / "Create tasks for related fixes"
...
|--------|-------------|------------|
| Investigation | Read error location + 1-2 callers | 2-3 parallel explorations |
| Hypotheses | Minimum 1 | Minimum 2-3 |
| Root cause testing | Manual verification | 1-3 parallel hypothesis investigations |
...
**What this component does:** Provides a systematic, hypothesis-driven debugging workflow with triage-based routing between quick-fix and deep investigation tracks.

**Capabilities needed:**
- File reading and searching (code investigation)
- File editing (applying fixes)
- Shell command execution (running tests, git commands)
- User interaction (track selection, fix approval)
- Background task delegation (deep track parallel exploration and investigation)

**Adaptation guidance:**
- Deep track investigation uses **code-explorer** from the core-tools package for parallel codebase exploration — adapt the delegation mechanism to your platform's task/agent system
- Deep track root cause analysis uses **bug-investigator** for parallel hypothesis testing — see the bug-investigator skill in this package
- Language-specific debugging references are in `references/python-debugging.md` and `references/typescript-debugging.md`
- The general debugging reference (originally a separate file) is inlined above for languages not covered by the specific references
