...
- Optional version override (e.g., `1.0.0`). If not provided, version is calculated from changelog entries.
...
Delegate to a **changelog-manager** agent:
...
Edit CHANGELOG.md with the transformed content.
...
**What this component does:** Automates a complete Python package release workflow: pre-flight checks (branch, clean state), test/lint/build verification, changelog analysis with optional agent-assisted updates, semantic version calculation, changelog transformation, and git tag creation.

**Capabilities needed:**
- File read/edit operations (CHANGELOG.md, pyproject.toml)
- Shell command execution (git, uv, ruff, pytest)
- User interaction (prompts, confirmations)
- Sub-agent delegation (changelog-manager agent for changelog updates)

**Adaptation guidance:**
- The tooling is Python-specific (uv, ruff, pytest) -- adapt Step 2-4 commands for other language ecosystems
- The changelog format follows Keep a Changelog; refer to the changelog-format skill for conventions
- The changelog-manager agent (Step 5) is optional -- the workflow works without it
- Steps 8-9 push directly to main and create tags -- ensure this matches your team's release process
