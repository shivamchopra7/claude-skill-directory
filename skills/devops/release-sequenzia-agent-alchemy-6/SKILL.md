...
All verification checks have passed. Before calculating the version, offer to run the changelog worker to ensure the `[Unreleased]` section is up-to-date.

Prompt the user:

```
Would you like to run the changelog worker to update CHANGELOG.md before proceeding?
...
Delegate to a changelog worker:
- Prompt: "Analyze commits since the last release and update the CHANGELOG.md [Unreleased] section"
- The worker will analyze commits, suggest entries, and update CHANGELOG.md after user approval
- Wait for the worker to complete before proceeding

**If user selects "No":**

Continue to Step 6 (Calculate Version) without running the changelog worker.
...
- **File system access**: Read and modify CHANGELOG.md and pyproject.toml
- **Shell execution**: Run shell commands (uv, ruff, git operations)
- **Parallel delegation**: Ability to delegate to a changelog worker (optional, for Step 5)
- **User interaction**: Prompt the user for version confirmation, changelog update decisions

### Adaptation Guidance

- This skill is specific to Python packages using `uv` and `ruff`. Adapt the test/lint/build commands for other ecosystems.
- The changelog worker in Step 5 is optional — the workflow works without it by using the existing changelog content.
- Git operations (commit, push, tag) require appropriate repository permissions.
