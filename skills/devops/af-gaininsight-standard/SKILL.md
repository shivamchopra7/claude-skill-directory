---
name: af-gaininsight-standard
description: |
  Use when setting up or working with the GainInsight Standard stack - a 4-layer opinionated
  infrastructure setup with AWS multi-account, Doppler secrets, Amplify hosting, and full
  dev/test/prod environments. Covers layer progression, validation, and teardown decisions.
  For credential access and DNS operations, this skill references `af-environment-infrastructure`.
...
- **`af-environment-infrastructure`** - For credential access, DNS operations, Route53, CloudFront setup
- **`af-cognito-expertise`** - For Cognito User Pool setup, custom attributes, auth triggers
- **`af-ses-expertise`** - For SES domain verification, sandbox/production, DKIM
- **`af-testing-expertise`** - For test framework configuration (Layer 2)
- **`af-documentation-standards`** - For documentation requirements
...
14. **SHOULD configure paired test scripts** for credential-dependent tests:
    ...
    - See `af-setup-process` skill for full script configuration
