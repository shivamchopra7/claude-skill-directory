...
description: Set up new AgentFlow projects and install modules after Discovery. Use when bootstrapping infrastructure, configuring Doppler, GitHub, Linear, or local environment variables.
...
**Steps:**
1. Create Linear team (via GraphQL API - see `af-query-linear-api`)
...
3. **For each module in dependency order:**
   a. **Load the module's skill** (e.g., `af-configure-cognito-auth` for auth)
...
**Full guide:** See [Auth Setup Guide](../../docs/guides/gaininsight-standard/auth-setup.md) for comprehensive Cognito + SES workflows. Skills: `af-configure-cognito-auth`, `af-configure-ses-email`.
...
**Related skills:**
- `af-discover-scope` - Next phase after setup
- `af-configure-test-frameworks` - For E2E infrastructure requirements
