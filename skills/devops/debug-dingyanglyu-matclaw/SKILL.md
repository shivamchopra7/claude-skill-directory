...
|-----|----------|---------|
| **Main app logs** | `logs/matclaw.log` | Host-side WhatsApp, routing, container spawning |
| **Main app errors** | `logs/matclaw.error.log` | Host-side errors |
...
echo '{}' | docker run -i \
  ...
  --entrypoint /bin/bash matclaw-agent:latest \
...
```bash
docker run --rm --entrypoint /bin/bash matclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
docker run --rm --entrypoint /bin/bash matclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
docker run --rm -it --entrypoint /bin/bash matclaw-agent:latest
```
...
# Also clear the session ID from MatClaw's tracking (stored in SQLite)
sqlite3 store/messages.db "DELETE FROM sessions WHERE group_folder = '{groupFolder}'"
```

To verify session resumption is working, check the logs for the same session ID across messages:
```bash
grep "Session initialized" logs/matclaw.log | tail -5
...
```bash
echo "=== Checking MatClaw Container Setup ==="
...
echo '{}' | docker run -i --entrypoint /bin/echo matclaw-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/matclaw.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
