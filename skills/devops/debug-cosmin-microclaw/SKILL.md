...
|-----|----------|---------|
| **Main app logs** | `logs/microclaw.log` | Host-side WhatsApp, routing, container spawning |
...
echo '{}' | container run -i \
  ...
  --entrypoint /bin/bash microclaw-agent:latest \
...
```bash
container run --rm --entrypoint /bin/bash microclaw-agent:latest -c 'ls -la /workspace/'
```
...
```bash
container run --rm --entrypoint /bin/bash microclaw-agent:latest -c '
  whoami
  ls -la /workspace/
  ls -la /app/
'
...
```bash
container run --rm -it --entrypoint /bin/bash microclaw-agent:latest
```
...
```bash
grep "Session initialized" logs/microclaw.log | tail -5
...
container system status &>/dev/null && echo "OK" || echo "NOT RUNNING - MicroClaw should auto-start it; check logs"

echo -e "\n4. Container image exists?"
echo '{}' | container run -i --entrypoint /bin/echo microclaw-agent:latest "OK" 2>/dev/null || echo "MISSING - run ./container/build.sh"
...
SESSIONS=$(grep "Session initialized" logs/microclaw.log 2>/dev/null | tail -5 | awk '{print $NF}' | sort -u | wc -l)
