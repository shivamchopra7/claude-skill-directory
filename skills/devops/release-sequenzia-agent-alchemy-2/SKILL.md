---
name: release
description: Prepare and execute a Python package release with verification steps. Use for releasing Python packages.
dependencies:
  - changelog-manager
---
...
Accept the following inputs: an optional version override (e.g., `1.0.0`). If not provided, version is calculated from changelog entries.
...
All verification checks have passed. Before calculating the version, offer to update the changelog to ensure the `[Unreleased]` section is up-to-date.

Prompt the user:

```
Would you like to update the changelog before proceeding?
...
Delegate to the **changelog-manager** skill to analyze commits and update CHANGELOG.md. Wait for completion before proceeding.

**If user selects "No":**

Continue to Step 6 (Calculate Version) without updating the changelog.
...
This skill was converted from the dev-tools plugin package. It orchestrates a Python package release workflow and delegates changelog updates to the **changelog-manager** skill. The workflow uses `uv` for package management and `ruff` for linting.
