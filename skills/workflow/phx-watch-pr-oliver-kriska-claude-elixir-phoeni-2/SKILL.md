...
```
/skill:phx-watch-pr 42                 # watch reviews + comments + checks
/skill:phx-watch-pr 42 --checks-only   # CI only (delegates to gh pr checks --watch)
/skill:phx-watch-pr 42 --fix           # on actionable review, draft fixes too
/skill:phx-watch-pr 42 --codex         # + request Codex cloud review, loop until clean
/skill:phx-watch-pr 42 --codex --codex-rounds 2   # cap re-review rounds (default 3)
```
...
4. **NEVER auto-post replies or auto-push** — hand off to `/skill:phx-pr-review`
...
`/skill:phx-review --codex` / `/skill:phx-codex-loop` run are DIFFERENT mechanisms from
...
report the conclusion; on failure, offer `/skill:phx-investigate` with the
...
| Event | Action |
|-------|--------|
| `review` / `comment` (actionable) | Summarize the delta; with `--fix` draft fixes + `mix compile && mix test`; route reply drafting to `/skill:phx-pr-review {n}` |
| `check` conclusion `failure` | Offer `/skill:phx-investigate` on the failing job |
| `codex_ack` | Note "codex is reviewing (~15–20 min on large PRs)"; keep waiting |
| `codex_review` | Stop the watcher. Run `/skill:phx-pr-review {n} --bots-only` (fix → reply → resolve; user approves and pushes). If rounds < `--codex-rounds`: post `@codex review` again, restart watcher with the new trigger id, round+1. Else: report remaining findings, stop |
...
A `codex_review` whose `/skill:phx-pr-review --bots-only` fetch finds zero
...
```text
push / open PR → /skill:phx-watch-pr {n} ──(new review)──► /skill:phx-pr-review {n}
                              ├──────(CI fail)─────► /skill:phx-investigate
                              ├──(--codex: codex_review)─► /skill:phx-pr-review --bots-only → push → re-request → watch (≤3 rounds)
