...
Note: Self-serving reviews are against guidelines. Reviews must be from real customers.
...
Use `@graph` for multiple schema types:
...
### Common Errors
- **Missing required properties** -- check Google's docs, not just schema.org minimum
- **Invalid values** -- dates in ISO 8601, URLs fully qualified, exact enum values
...
- Component that renders schema as JSON-LD in a `<script type="application/ld+json">` tag
- Server-side rendered for SEO
- Sanitize any user-provided data before embedding
...
Provide complete JSON-LD code block ready to paste.
