---
name: af-generate-quotations
description: Generate client-facing quotations from Linear estimation data. Use when calculating pricing, producing PDF quotes, or formatting quote templates from effort breakdowns.
...
related:
  ...
  - ../af-send-notifications/SKILL.md
  - ../af-send-emails-gmail/SKILL.md
---
...
12. **Email the PDF using af-send-emails-gmail.** After generating the PDF, send it to the recipient via Gmail API. Load the email-sending skill for the sending workflow.
...
Load the **af-send-emails-gmail** skill and send the PDF to the recipient:
...
**Email sending** (af-send-emails-gmail):
...
5. Email the PDF via `af-send-emails-gmail` (Gmail API, sends as holly@gaininsight.global)
