...
This document defines the public JSON artifact surface for directory consumers.
The artifacts are generated by the pipeline in `scripts/` and committed to
`shivamchopra7/claude-skill-directory`; the generated files are publish outputs,
not source files to patch by hand.

## Locations

- Pages base URL: `https://shivamchopra7.github.io/claude-skill-directory/`
- Raw base URL:
  `https://raw.githubusercontent.com/shivamchopra7/claude-skill-directory/main/`

Pages is optimized for the web app and public read APIs. The raw repository
tree is optimized for bulk registry compatibility and raw archive browsing.
Both are served from the same single repository.

## Public Entry Points

| Path | Location | Contract |
| --- | --- | --- |
| `search-index-lite.json` | Pages | Startup catalog index with deduped summaries and quality/security signals. |
| `search-index.json` | Pages | Compatibility pointer for the full search payload. |
| `search-index-manifest.json` | Pages | Full search shard manifest. |
| `search-shards/part-000.json` | Pages | Full search shard part pattern. |
| `featured.json` | Pages | Featured skills for the web app. |
| `plugins.json` | Pages | Plugin catalog. |
| `stats.json` | Pages | Generated counts, shard sizes, and summary statistics. |
| `quality-index.json` | Pages | Compatibility pointer for quality records. |
| `quality-index-manifest.json` | Pages | Quality shard manifest. |
| `quality-shards/part-000.json` | Pages | Quality shard part pattern. |
| `security-index.json` | Pages | Compatibility pointer for security records. |
| `security-index-manifest.json` | Pages | Security shard manifest. |
| `security-shards/part-000.json` | Pages | Security shard part pattern. |
| `ranking-index.json` | Pages | Compatibility pointer for ranking records. |
| `ranking-index-manifest.json` | Pages | Ranking shard manifest. |
| `ranking-shards/part-000.json` | Pages | Ranking shard part pattern. |
| `categories/index.json` | Pages | Category manifest index. |
| `categories/<category>.json` | Pages | Legacy category compatibility pointer. |
| `categories/<category>/manifest.json` | Pages | Category part manifest. |
| `categories/<category>/part-000.json` | Pages | Category part pattern. |
| `registry_summary.json` | Raw | Lightweight source summary and counts. |
| `registry.json` | Raw | Compatibility pointer for the full registry payload. |
| `registry-manifest.json` | Raw | Registry shard manifest. |
| `registry-shards/00.json` | Raw | Registry shard pattern, keyed by two-character shard id. |

Any new public JSON artifact must be added to this table in the same PR that
adds or changes the generated artifact.

Removed in this version: `provenance/merge-source.json`. It existed only to
pin the source refs of a cross-repository merge. The project is now a single
self-contained repository, so there is no second ref to pin and the file is no
longer published. Consumers that read it should drop the dependency; the
publishing commit itself is the provenance.
...
giant single-payload JSON files. Load `registry-manifest.json` and every shard
it lists; `registry.json` is compatibility-only. Missing manifests or shards, invalid JSON, and malformed
...
manifest_url = (
    "https://raw.githubusercontent.com/shivamchopra7/"
    "claude-skill-directory/main/registry-manifest.json"
)
